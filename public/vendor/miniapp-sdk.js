...
function createEncoder(encoder2) {
  return Object.freeze({
    ...encoder2,
    encode: (value) => {
      const bytes2 = new Uint8Array(getEncodedSize(value, encoder2));
      encoder2.write(value, bytes2, 0);
      return bytes2;
    }
  });
}
function createDecoder(decoder2) {
  return Object.freeze({
    ...decoder2,
    decode: (bytes2, offset2 = 0) => decoder2.read(bytes2, offset2)[0]
  });
}
...
function unserializeTransaction(transaction) {
  const bytes2 = Buffer.from(transaction, "base64");
  const version3 = VersionedMessage.deserializeMessageVersion(bytes2);
  return version3 === "legacy" ? VersionedTransaction.deserialize(bytes2) : Transaction.from(bytes2);
}
